...
const getTokenAndOwner = async function(url) {
  if(!url)
    throw new Error('function getTokenAndOwner requires param url!');
...
// @return {Array}
const formatTasksFromSqlToLocal = function(tasksSql = [], listsLocal = [], {owner, token}) {
  if(!owner || !token) 
    throw new Error('function "formatTasksFromSqlToLocal" requires param {token, owner}');
...
const formatListsFromLocalToSql = function(listsLocal, {owner}) {
  if(!owner)
    throw new Error('function formatListsFromLocalToSql requires param {owner}!');
...
// @param {Object} {owner}
// @return {Array}
const formatTasksFromLocalToSql = function(tasksLocal, listsLocal, {owner, token}) {
  if(!owner || !token)
    return new Error('function "formatTasksFromLocalToSql" requires param {owner, token}');
